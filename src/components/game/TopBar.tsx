...
export const TopBar = React.memo(function TopBar() {
  ...
  const m = useMessages();
  const statsWithVc = (stats ?? {}) as Partial<typeof stats> & { valuation?: number; headcount?: number };
  const toNum = (value: unknown, fallback = 0): number =>
    typeof value === 'number' && Number.isFinite(value) ? value : fallback;
  const valuation = toNum(statsWithVc.valuation, toNum(statsWithVc.population, 0));
  const headcount = toNum(statsWithVc.headcount, toNum(statsWithVc.jobs, 0));
  const money = toNum(statsWithVc.money, 0);
  const income = toNum(statsWithVc.income, 0);
  const expenses = toNum(statsWithVc.expenses, 0);
  const netBurn = income - expenses;
  const demand = {
    residential: toNum(statsWithVc.demand?.residential, 0),
    commercial: toNum(statsWithVc.demand?.commercial, 0),
    industrial: toNum(statsWithVc.demand?.industrial, 0),
  };
  ...
  return (
    <div className="h-14 bg-card border-b border-border flex items-center justify-between px-4">
      ...
      <div className="flex items-center gap-3">
        <StatBadge value={valuation.toLocaleString()} label={String(m(UI_LABELS.valuation))} />
        <StatBadge value={headcount.toLocaleString()} label={String(m(UI_LABELS.headcount))} />
        <StatBadge 
      ...
      <div className="flex items-center gap-2">
        <StatBadge 
          ...
          label={String(m(UI_LABELS.burn))}
          variant={netBurn >= 0 ? 'success' : 'destructive'}
        />
