...
import { VCSimDashboard } from '@/components/VCSimDashboard';
  ...
  try {
    ...
    if (saved) {
      ...
      if (Array.isArray(parsed)) {
        return parsed
          .filter((city): city is Partial<SavedCityMeta> => !!city && typeof city === 'object')
          .map((city, index) => ({
  ...
  return (
    <MultiplayerContextProvider>
      <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-8">
        <div className="max-w-7xl w-full grid lg:grid-cols-[1fr_auto_1fr] gap-16 items-start">
          ...
          {/* Middle - VC Simulation Dashboard (Visual Preview) */}
