...
"[project]/src/lib/investorContext.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "buildInvestorContextSql",
    ()=>buildInvestorContextSql,
    "resolveInvestorContext",
    ()=>resolveInvestorContext
]);
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = /*#__PURE__*/ __turbopack_context__.i("[project]/node_modules/next/dist/build/polyfills/process.js [app-client] (ecmascript)");
const VC_API_BASE_URL = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].env.NEXT_PUBLIC_VC_SIM_API_BASE_URL?.replace(/\/$/, '') || 'http://localhost:8000';
...
function buildInvestorContextSql(domain) {
    const normalized = normalizeDomain(domain);
    const escapedDomain = sqlEscape(normalized);
    return `
WITH selected_investor AS (
  SELECT i.id, i.domain, i.fund_name, i.vintage_year, i.focus_stage, i.thesis, i.aum_usd,
         i.month_rank, i.lead_ratio, i.follow_on_rate
  FROM investors i
  WHERE lower(i.domain) = '${escapedDomain}'
  LIMIT 1
),
portfolio_rows AS (
  SELECT c.id, c.name, c.domain, c.sector, c.stage, inv.last_round_usd,
         ci.pmf_signal, ci.growth_momentum, ci.burn_discipline,
         ci.investor_confidence, ci.hiring_velocity, lm.logo_url
  FROM selected_investor si
  JOIN investments inv ON inv.investor_id = si.id
  JOIN companies c ON c.id = inv.company_id
  LEFT JOIN company_insights ci ON ci.company_id = c.id
  LEFT JOIN vc_company_logo_manifest lm ON lm.company_domain = c.domain
  WHERE inv.status IN ('active', 'follow_on')
  ORDER BY inv.entry_date DESC NULLS LAST
  LIMIT 40
),
competitor_rows AS (
  SELECT ci2.domain, ci2.fund_name,
         coalesce(avg(ce.overlap_score), 0)::float AS overlap_score,
         count(*) FILTER (WHERE ce.status = 'contested')::int AS active_contests
  FROM selected_investor si
  JOIN competitor_edges ce ON ce.investor_id = si.id
  JOIN investors ci2 ON ci2.id = ce.competitor_investor_id
  GROUP BY ci2.domain, ci2.fund_name
...
SELECT json_build_object(
  ...
  coalesce(
    (SELECT json_agg(
      json_build_object(
        ...
        'sector', pr.sector,
        'stage', pr.stage,
        'lastRoundUsd', pr.last_round_usd,
        'pmfSignal', pr.pmf_signal,
        'growthMomentum', pr.growth_momentum,
        'burnDiscipline', pr.burn_discipline,
        'investorConfidence', pr.investor_confidence,
        'hiringVelocity', pr.hiring_velocity,
        'logoUrl', pr.logo_url
      )
  ...
  coalesce(
    (SELECT json_agg(
      json_build_object(
        'domain', cr.domain,
        'fundName', cr.fund_name,
        'overlapScore', cr.overlap_score,
        'activeContests', cr.active_contests
      )
    ) FROM competitor_rows cr),
    '[]'::json
  )
) AS investor_session_context;
`.trim();
}
async function resolveInvestorContext(domainInput) {
    const domain = normalizeDomain(domainInput);
    try {
        const response = await fetch(`${VC_API_BASE_URL}/api/v1/investors/search?domain=${encodeURIComponent(domain)}`, {
            cache: 'no-store'
        });
        if (response.ok) {
            const payload = await response.json();
            const profile = payload.profile ?? {};
            const portfolioRows = Array.isArray(payload.portfolio_companies) ? payload.portfolio_companies : [];
            if (typeof profile.domain === 'string' && typeof profile.name === 'string') {
                return {
                    profile: {
                        domain: profile.domain,
                        fundName: profile.name,
                        vintageYear: typeof profile.founded_year === 'number' ? profile.founded_year : 2018,
                        focusStage: 'seed',
                        thesis: typeof profile.thesis === 'string' ? profile.thesis : 'Founder-led software and frontier tech with fast iteration loops.',
                        aumUsd: typeof profile.aum_usd === 'number' ? profile.aum_usd : 350000000,
                        monthRank: typeof profile.march_rank === 'number' ? profile.march_rank : 10,
                        leadRatio: typeof profile.lead_investment_ratio_pct === 'number' ? Number((profile.lead_investment_ratio_pct / 100).toFixed(2)) : 0.45,
                        followOnRate: typeof profile.follow_on_rate_pct === 'number' ? Number((profile.follow_on_rate_pct / 100).toFixed(2)) : 0.33
                    },
                    portfolio: portfolioRows.slice(0, 20).map((row, index)=>({
                            id: typeof row.id === 'string' ? row.id : `portfolio-${index + 1}`,
                            name: typeof row.name === 'string' ? row.name : `Portfolio ${index + 1}`,
                            domain: typeof row.domain === 'string' ? row.domain : `portfolio-${index + 1}.vc`,
                            sector: typeof row.sector === 'string' ? row.sector : 'Software',
                            stage: typeof row.stage === 'string' ? row.stage : 'seed',
                            lastRoundUsd: typeof row.valuation_usd === 'number' ? row.valuation_usd : 12000000 + index * 1000000,
                            growthMomentum: typeof row.growth_mom_pct === 'number' ? row.growth_mom_pct : 65 + index % 10,
                            pmfSignal: 64 + index % 12,
                            burnDiscipline: 58 + index % 9,
                            investorConfidence: 60 + index % 11,
                            hiringVelocity: 52 + index % 13
                        })),
                    competitors: [
                        {
    ...
    // Fall back to deterministic local context.
    }
    if (domain === 'ycombinator.com') {
        return {
            profile: {
                domain: 'ycombinator.com',
                fundName: 'Y Combinator Continuity',
                vintageYear: 2015,
                focusStage: 'seed',
                thesis: 'Founder-led software and frontier tech with fast iteration loops.',
                aumUsd: 12000000000,
                monthRank: 1,
                leadRatio: 0.62,
                followOnRate: 0.71
    ...
    const fallbackSeed = domain.split('').reduce((acc, ch)=>(acc + ch.charCodeAt(0)) % 997, 131);
    const ratio = (fallbackSeed % 35 + 45) / 100;
    const followOn = (fallbackSeed % 28 + 40) / 100;
    return {
        profile: {
            domain,
            fundName: `${domain.split('.')[0] || 'Emerging'} Ventures`,
            vintageYear: 2018 + fallbackSeed % 6,
            focusStage: 'seed',
            thesis: 'Operator-led B2B software, AI applications, and infrastructure resilience.',
            aumUsd: 250000000 + fallbackSeed % 12 * 75000000,
            monthRank: 8 + fallbackSeed % 22,
            leadRatio: Number(ratio.toFixed(2)),
            followOnRate: Number(followOn.toFixed(2))
        },
        portfolio: [
            {
                id: `${domain}-p1`,
                name: 'Atlas Forge',
                domain: 'atlasforge.io',
                sector: 'DevTools',
                stage: 'series-a',
                lastRoundUsd: 42000000,
                growthMomentum: 74,
                pmfSignal: 77,
                burnDiscipline: 63,
                investorConfidence: 71,
                hiringVelocity: 58
            },
            {
                id: `${domain}-p2`,
                name: 'Relay HealthOps',
                domain: 'relayhealthops.com',
                sector: 'Healthtech',
                stage: 'seed',
                lastRoundUsd: 16000000,
                growthMomentum: 68,
                pmfSignal: 73,
                burnDiscipline: 66,
...
"[project]/src/lib/investorProfile.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "INVESTOR_DOMAIN_STORAGE_KEY",
    ()=>INVESTOR_DOMAIN_STORAGE_KEY,
    "SQL_QUERY_BLUEPRINTS",
    ()=>SQL_QUERY_BLUEPRINTS,
    "formatCompactUsd",
    ()=>formatCompactUsd,
...
const SQL_QUERY_BLUEPRINTS = {
    investorByDomain: `-- investorByDomain
select
  i.id,
  i.name,
  i.domain,
...
select
  ...
  c.domain,
  c.sector,
  p.last_round_size_usd,
  p.last_round_at
from investor_portfolio p
join companies c on c.id = p.company_id
where p.investor_id = $1
order by p.last_round_at desc
limit $2;`,
    investorCompetitors: `-- investorCompetitors
select
  c.investor_id,
  c.competitor_name,
  c.competitor_domain,
  c.overlap_score,
  c.recent_competitive_rounds
from investor_competitors c
where c.investor_id = $1
order by c.overlap_score desc
limit 10;`,
    portfolioPercentiles: `-- portfolioPercentiles
select
  percentile_disc(0.5) within group (order by pmf_signal) as pmf_signal_p50,
  percentile_disc(0.5) within group (order by growth_momentum) as growth_momentum_p50,
  percentile_disc(0.5) within group (order by burn_discipline) as burn_discipline_p50,
  percentile_disc(0.5) within group (order by investor_confidence) as investor_confidence_p50,
  percentile_disc(0.5) within group (order by hiring_velocity) as hiring_velocity_p50
...
const YC_PROFILE = {
    ...
    marchRank: 1,
    aumUsd: 17000000000,
    dryPowderUsd: 4200000000,
    leadPercentage: 36,
    followOnPercentage: 44,
    recentInvestments: 38,
    monthlyDealFlow: 24,
    portfolioCompanies: [
        {
            name: 'Stripe',
            domain: 'stripe.com',
            seedBuilding: 'office_building_small'
        },
        {
            name: 'Airbnb',
            domain: 'airbnb.com',
            seedBuilding: 'shop_medium'
        },
        {
            name: 'Brex',
            domain: 'brex.com',
            seedBuilding: 'office_low'
        },
        {
            name: 'DoorDash',
            domain: 'doordash.com',
        ...
        },
        {
            name: 'Coinbase',
            domain: 'coinbase.com',
            seedBuilding: 'office_high'
        },
        {
            name: 'Instacart',
            domain: 'instacart.com',
            seedBuilding: 'warehouse'
        }
    ],
...
const FALLBACK_PROFILE = {
    ...
    marchRank: 48,
    aumUsd: 1200000000,
    dryPowderUsd: 280000000,
    leadPercentage: 21,
    followOnPercentage: 34,
    ...
    portfolioCompanies: [
        ...
        },
        {
            ...
            seedBuilding: 'hospital'
        },
        {
            name: 'Harbor Robotics',
            domain: 'harborrobotics.com',
            seedBuilding: 'industrial'
        },
        {
            name: 'Urban Relay',
            domain: 'urbanrelay.com',
            seedBuilding: 'warehouse'
        }
    ],
    portfolioPercentiles: {
        ...
        growthMomentum: 57,
        burnDiscipline: 66,
        investorConfidence: 59,
        hiringVelocity: 54
    }
};
function normalizeDomain(input) {
    const value = input.trim().toLowerCase();
    if (!value) return YC_PROFILE.domain;
    return value.replace(/^https?:\/\//, '').replace(/\/.*$/, '');
}
function domainToDisplayName(domain) {
    const withoutTld = domain.split('.')[0] || 'Frontier';
    return withoutTld.split(/[-_]/g).filter(Boolean).map((word)=>word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
}
function resolveInvestorProfile(domainInput) {
    const domain = normalizeDomain(domainInput);
    ...
    return {
        ...FALLBACK_PROFILE,
        domain,
        name: `${domainToDisplayName(domain)} Ventures`
    };
}
...
//# sourceMappingURL=src_lib_1353bd50._.js.map