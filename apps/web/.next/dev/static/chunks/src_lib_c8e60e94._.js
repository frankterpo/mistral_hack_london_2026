...
"[project]/src/lib/saveWorker.ts (static in ecmascript)", ((__turbopack_context__) => {

__turbopack_context__.v("/_next/static/media/saveWorker.8835aa78.ts");}),
"[project]/src/lib/saveWorker.ts [app-client] (ecmascript, worker loader)", ((__turbopack_context__) => {

__turbopack_context__.v(__turbopack_context__.b([
  "static/chunks/_4fba9bcb._.js",
  "static/chunks/src_lib_saveWorker_ts_b3f8fcd6._.js",
  "static/chunks/turbopack-src_lib_saveWorker_ts_d551f5b2._.js"
]));
}),
"[project]/src/lib/saveWorkerManager.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "compressAsync",
    ()=>compressAsync,
    "decompressAndParseAsync",
    ()=>decompressAndParseAsync,
    "serializeAndCompressAsync",
    ()=>serializeAndCompressAsync,
    "serializeAndCompressForDBAsync",
    ()=>serializeAndCompressForDBAsync,
    "terminateWorker",
    ()=>terminateWorker
]);
...
const __TURBOPACK__import$2e$meta__ = {
    get url () {
        return `file://${__turbopack_context__.P("src/lib/saveWorkerManager.ts")}`;
    }
};
;
let worker = null;
 ...
 */ function initWorker() {
    ...
    try {
        ...
        worker = new Worker(__turbopack_context__.r("[project]/src/lib/saveWorker.ts [app-client] (ecmascript, worker loader)"));
...
async function serializeAndCompressAsync(state) {
    // Try to initialize worker if not already done
    if (!worker && !initWorker()) {
        // Fallback: serialize and compress on main thread
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lz$2d$string$2f$libs$2f$lz$2d$string$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["compressToUTF16"])(JSON.stringify(state));
    }
    const id = ++requestId;
    return new Promise((resolve, reject)=>{
        const timeoutId = setTimeout(()=>{
            ...
            if (pending) {
                ...
                try {
                    const compressed = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lz$2d$string$2f$libs$2f$lz$2d$string$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["compressToUTF16"])(JSON.stringify(state));
                    resolve(compressed);
                } catch (error) {
                    reject(error);
                }
        ...
        try {
            ...
            worker.postMessage({
                type: 'compress-transferred',
                id,
                buffer
            }, [
                buffer
            ] // Transfer list - buffer ownership moves to worker
            );
        } catch (error) {
...
async function decompressAndParseAsync(compressed) {
    // Try to initialize worker if not already done
    if (!worker && !initWorker()) {
...
async function serializeAndCompressForDBAsync(state) {
    // Try to initialize worker if not already done
    if (!worker && !initWorker()) {
        // Fallback: serialize and compress on main thread
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lz$2d$string$2f$libs$2f$lz$2d$string$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["compressToEncodedURIComponent"])(JSON.stringify(state));
    }
    const id = ++requestId;
    return new Promise((resolve, reject)=>{
        const timeoutId = setTimeout(()=>{
            ...
            if (pending) {
                ...
                console.warn('Save worker timeout, falling back to main thread');
                try {
                    const compressed = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lz$2d$string$2f$libs$2f$lz$2d$string$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["compressToEncodedURIComponent"])(JSON.stringify(state));
                    resolve(compressed);
                } catch (error) {
                    reject(error);
                }
...
"[project]/src/lib/names.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function generateWaterName(type) {
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$names$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/names.ts [app-client] (ecmascript)");
...
;
;
const DEFAULT_GRID_SIZE = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$react$2d$device$2d$detect$2f$dist$2f$lib$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["isMobile"] ? 50 : 70;
// In VC mode, fire incidents are only used as a visual "integration shock"
// during short windows after acquisition/M&A events.
const ACQUISITION_FIRE_WINDOW_MS = 2 * 60 * 1000;
function isAcquisitionTriggeredNotification(notification) {
    const haystack = `${notification.id} ${notification.title} ${notification.description}`.toLowerCase();
    return haystack.includes('acquisition') || haystack.includes('acquire') || haystack.includes('merger') || haystack.includes('buyout') || haystack.includes('m&a');
}
...
function isFarmBuilding(x, y, buildingType) {
...
function isStarterBuilding(x, y, buildingType) {
...
function perlinNoise(x, y, seed, octaves = 4) {
    let total = 0;
    let frequency = 0.05;
    let amplitude = 1;
    let maxValue = 0;
...
function generateLakes(grid, size, seed) {
    // Use noise to find potential lake centers - look for low points
    const lakeNoise = (x, y)=>perlinNoise(x, y, seed + 1000, 3);
    ...
    for (const center of selectedCenters){
        ...
        while(lakeTiles.length < targetSize && candidates.length > 0){
            ...
            for (const [dx, dy] of directions){
                ...
                if (nx >= minDistFromEdge && nx < size - minDistFromEdge && ny >= minDistFromEdge && ny < size - minDistFromEdge && !lakeTiles.some((t)=>t.x === nx && t.y === ny) && !candidates.some((c)=>c.x === nx && c.y === ny)) {
        ...
        let lakeName = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$names$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generateWaterName"])('lake');
        while(usedLakeNames.has(lakeName)){
            lakeName = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$names$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generateWaterName"])('lake');
        }
...
function generateOceans(grid, size, seed) {
    const waterBodies = [];
    const oceanChance = 0.4; // 40% chance per edge
    // Use noise for coastline variation
    const coastNoise = (x, y)=>perlinNoise(x, y, seed + 2000, 3);
    ...
    const generateOceanEdge = (isHorizontal, edgePosition, inwardDirection // 1 = increasing coord, -1 = decreasing coord
    )=>{
        const tiles = [];
    ...
    for (const edge of edges){
        if (edge.tiles.length > 0) {
            ...
            let oceanName = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$names$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generateWaterName"])('ocean');
            while(usedOceanNames.has(oceanName)){
                oceanName = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$names$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generateWaterName"])('ocean');
            }
...
function generateAdjacentCities() {
    const cities = [];
    const directions = [
        'north',
        'south',
    ...
    for (const direction of directions){
        let name;
        do {
            name = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$names$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generateCityName"])();
...
function hasRoadAtEdge(grid, gridSize, direction) {
    switch(direction){
        case 'north':
            // Check top edge (y = 0)
...
function checkForDiscoverableCities(grid, gridSize, adjacentCities) {
    const citiesToShow = [];
...
function getConnectableCities(grid, gridSize, adjacentCities) {
    const connectable = [];
    for (const city of adjacentCities){
...
// Generate terrain - concrete-like buildable base with scattered trees.
// Water obstacles are intentionally disabled for ISOBUSINESS so map area
// pressure comes from grid size / expansion decisions.
function generateTerrain(size) {
    ...
    for(let y = 0; y < size; y++){
        for(let x = 0; x < size; x++){
            ...
            // Also add some tree clustering for visual variation
...
function isNearWater(grid, x, y, size) {
...
const WATERFRONT_BUILDINGS = [
    'marina_docks_small',
    'pier_large'
];
function requiresWaterAdjacency(buildingType) {
    return WATERFRONT_BUILDINGS.includes(buildingType);
}
function getWaterAdjacency(grid, x, y, width, height, gridSize) {
...
function getRoadAdjacency(grid, x, y, width, height, gridSize) {
    // In isometric view (looking from SE toward NW):
    // - The default sprite faces toward the "front" (south-east in world coords)
    // - To face the opposite direction, we flip horizontally
...
// ============================================================================
/** Maximum width of water a bridge can span */ const MAX_BRIDGE_SPAN = 10;
/** Bridge type thresholds based on span width */ const BRIDGE_TYPE_THRESHOLDS = {
    large: 5,
    suspension: 10
};
/** Get the appropriate bridge type for a given span */ function getBridgeTypeForSpan(span) {
    // 1-tile bridges are simple bridges without trusses
    if (span === 1) return 'small';
    if (span <= BRIDGE_TYPE_THRESHOLDS.large) return 'large';
    return 'suspension';
}
/** Number of variants per bridge type */ const BRIDGE_VARIANTS = {
    small: 3,
    medium: 3,
    large: 2,
    suspension: 2
};
/** Generate a deterministic variant based on position */ function getBridgeVariant(x, y, bridgeType) {
    const seed = (x * 31 + y * 17) % 100;
    return seed % BRIDGE_VARIANTS[bridgeType];
}
/** Create a bridge building with all metadata */ function createBridgeBuilding(bridgeType, orientation, variant, position, index, span, trackType = 'road') {
    return {
        type: 'bridge',
        level: 0,
        population: 0,
...
/** Check if a tile at position is water */ function isWaterTile(grid, gridSize, x, y) {
    if (x < 0 || y < 0 || x >= gridSize || y >= gridSize) return false;
    return grid[y][x].building.type === 'water';
}
/** Check if a tile at position is a road or bridge */ function isRoadOrBridgeTile(grid, gridSize, x, y) {
    if (x < 0 || y < 0 || x >= gridSize || y >= gridSize) return false;
    const type = grid[y][x].building.type;
    return type === 'road' || type === 'bridge';
}
/** Scan for a bridge opportunity in a specific direction */ function scanForBridgeInDirection(grid, gridSize, startX, startY, dx, dy, orientation, trackType) {
    const waterTiles = [];
    let x = startX + dx;
    let y = startY + dy;
...
/** Detect if placing a road or rail creates a bridge opportunity from this tile */ function detectBridgeOpportunity(grid, gridSize, x, y, trackType) {
    const tile = grid[y]?.[x];
    if (!tile) return null;
...
/** Build bridges by converting water tiles to bridge tiles */ function buildBridges(grid, opportunity) {
...
/** Check and create bridges after road or rail placement */ function checkAndCreateBridges(grid, gridSize, placedX, placedY, trackType) {
...
function createBridgesOnPath(state, pathTiles, trackType = 'road') {
...
function createInitialGameState(size = DEFAULT_GRID_SIZE, cityName = 'New City') {
    ...
    return {
        ...
        competitionDegree: 32,
        pendingOpportunities: [],
        llmRouterMode: 'unknown'
    };
}
// Service building configuration - defined once, reused across calls
// Exported so overlay rendering can access radii
const withRange = (range, extra)=>({
        range,
        rangeSquared: range * range,
        ...extra
    });
const SERVICE_CONFIG = {
    police_station: withRange(13, {
        type: 'police'
    }),
    fire_station: withRange(18, {
        type: 'fire'
    }),
    hospital: withRange(24, {
        type: 'health'
    }),
    school: withRange(11, {
        type: 'education'
    }),
    university: withRange(19, {
        type: 'education'
    }),
...
const SERVICE_RANGE_INCREASE_PER_LEVEL = 0.2; // 20% per level (Level 1: 100%, Level 5: 180%)
const SERVICE_UPGRADE_COST_BASE = 2; // Cost = baseCost * (2 ^ currentLevel)
// Calculate service coverage from service buildings - optimized version
function calculateServiceCoverage(grid, size) {
    ...
    for (const building of serviceBuildings){
        ...
        if (type === 'power_plant') {
            ...
            const currentCoverage = services[serviceType];
            for(let ny = minY; ny <= maxY; ny++){
                for(let nx = minX; nx <= maxX; nx++){
                    const dx = nx - x;
                    const dy = ny - y;
...
function upgradeServiceBuilding(state, x, y) {
    ...
    const baseCost = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$game$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["TOOL_INFO"][buildingType]?.cost;
...
function canSpawnMultiTileBuilding(grid, x, y, width, height, zone, gridSize) {
...
function hasRoadAccess(grid, x, y, size, maxDistance = 8) {
...
function evolveBuilding(grid, x, y, services, demand) {
    const tile = grid[y][x];
    const building = tile.building;
    ...
    const zoneDemandValue = demand ? zone === 'residential' ? demand.residential : zone === 'commercial' ? demand.commercial : zone === 'industrial' ? demand.industrial : 0 : 0;
    ...
    const buildingList = zone === 'residential' ? __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["RESIDENTIAL_BUILDINGS"] : zone === 'commercial' ? __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["COMMERCIAL_BUILDINGS"] : zone === 'industrial' ? __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["INDUSTRIAL_BUILDINGS"] : [];
    ...
    const zoneDemandForLevel = demand ? zone === 'residential' ? demand.residential : zone === 'commercial' ? demand.commercial : zone === 'industrial' ? demand.industrial : 0 : 0;
    ...
    const isSmallResidential = zone === 'residential' && (building.type === 'house_small' || building.type === 'house_medium');
    const isSmallCommercial = zone === 'commercial' && (building.type === 'shop_small' || building.type === 'shop_medium');
    const isSmallIndustrial = zone === 'industrial' && building.type === 'factory_small';
    // Get relevant demand for this zone
    const zoneDemand = demand ? zone === 'residential' ? demand.residential : zone === 'commercial' ? demand.commercial : zone === 'industrial' ? demand.industrial : 0 : 0;
    if (zoneDemand > 30) {
    ...
    const buildingStats = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BUILDING_STATS"][anchorBuilding.type];
...
function calculateStats(grid, size, budget, taxRate, effectiveTaxRate, services) {
    let population = 0;
    let jobs = 0;
    ...
    const happiness = Math.min(100, safety * 0.15 + health * 0.2 + education * 0.15 + environment * 0.15 + jobSatisfaction * 0.2 + (100 - taxRate * 3) * 0.15);
    return {
        population,
        jobs,
        money: 0,
...
function updateBudgetCosts(grid, budget) {
    const newBudget = {
        ...budget
    };
    let policeCount = 0;
...
function generateAdvisorMessages(stats, services, grid) {
    const messages = [];
    ...
    if (unpoweredBuildings > 0) {
        messages.push({
            ...
            icon: 'power',
            messages: [
                `${unpoweredBuildings} buildings lack power. Build more power plants!`
            ],
    ...
    if (unwateredBuildings > 0) {
        messages.push({
            ...
            icon: 'water',
            messages: [
                `${unwateredBuildings} buildings lack water. Build water towers!`
            ],
    ...
    if (netIncome < 0) {
        messages.push({
            ...
            icon: 'cash',
            messages: [
                `City is running a deficit of $${Math.abs(netIncome)}/month. Consider raising taxes or cutting services.`
            ],
    ...
    if (stats.safety < 40) {
        messages.push({
            ...
            icon: 'shield',
            messages: [
                'Crime is on the rise. Build more police stations to protect citizens.'
            ],
    ...
    if (stats.health < 50) {
        messages.push({
            ...
            icon: 'hospital',
            messages: [
                'Health services are lacking. Build hospitals to improve citizen health.'
            ],
    ...
    if (stats.education < 50) {
        messages.push({
            ...
            icon: 'education',
            messages: [
                'Education levels are low. Build schools and universities.'
            ],
    ...
    if (stats.environment < 40) {
        messages.push({
            ...
            icon: 'environment',
            messages: [
                'Pollution is high. Plant trees and build parks to improve air quality.'
            ],
    ...
    if (stats.population > 100 && jobRatio < 0.8) {
        messages.push({
            ...
            icon: 'jobs',
            messages: [
                `Unemployment is high. Zone more commercial and industrial areas.`
            ],
...
function simulateTick(state) {
    ...
    const now = Date.now();
    const acquisitionFireActive = state.notifications.some((notification)=>isAcquisitionTriggeredNotification(notification) && now - notification.timestamp <= ACQUISITION_FIRE_WINDOW_MS);
    ...
    for(let y = 0; y < size; y++){
        for(let x = 0; x < size; x++){
            ...
            if (originalTile.zone === 'none' && (originalBuilding.type === 'grass' || originalBuilding.type === 'tree') && !needsPowerWaterUpdate && originalTile.pollution < 0.01 && (__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BUILDING_STATS"][originalBuilding.type]?.pollution || 0) === 0) {
                continue;
            }
            ...
            if (isCompletedServiceBuilding && !needsPowerWaterUpdate && originalTile.pollution < 0.01) {
                // Apply thematic boosts for completed service/park buildings
                if (originalBuilding.type === 'school' || originalBuilding.type === 'university') {
                    // Boost PMF Signal (Education)
                    state.stats.education = Math.min(100, state.stats.education + 0.05);
                } else if (originalBuilding.type === 'stadium' || originalBuilding.type === 'amusement_park') {
                    // Boost Growth Momentum (Safety/Demand)
                    state.stats.safety = Math.min(100, state.stats.safety + 0.05);
                    if (state.stats.demand) {
                        state.stats.demand.commercial = Math.min(100, state.stats.demand.commercial + 0.1);
                    }
                } else if (originalBuilding.type === 'power_plant' || originalBuilding.type === 'water_tower') {
                    // Event Collaboration and Commercial Partnership boost Growth Momentum (Safety)
                    state.stats.safety = Math.min(100, state.stats.safety + 0.03);
                    // Sustained coverage boosts Hiring Velocity (Health)
                    // We check if coverage is already high to simulate "sustained"
                    if (state.stats.safety > 80) {
                        state.stats.health = Math.min(100, state.stats.health + 0.02);
                    }
                } else if (originalBuilding.type === 'road' || originalBuilding.type === 'bridge') {
                    // Offering Partnership boosts PMF Signal (Education)
                    state.stats.education = Math.min(100, state.stats.education + 0.01);
                }
                continue;
            }
            ...
            // Hard-stop legacy city disasters in VC mode unless we are in an
            // acquisition-triggered incident window.
            if (!acquisitionFireActive && tile.building.onFire) {
                tile.building.onFire = false;
                tile.building.fireProgress = 0;
            }
            // Apply thematic boosts for active buildings
            if (tile.building.constructionProgress === 100) {
                if (tile.building.type === 'school' || tile.building.type === 'university') {
                    state.stats.education = Math.min(100, state.stats.education + 0.02);
                } else if (tile.building.type === 'stadium' || tile.building.type === 'amusement_park') {
                    state.stats.safety = Math.min(100, state.stats.safety + 0.02);
                    if (state.stats.demand) {
                        state.stats.demand.commercial = Math.min(100, state.stats.demand.commercial + 0.05);
                    }
                } else if (tile.building.type === 'power_plant' || tile.building.type === 'water_tower') {
                    // Event Collaboration and Commercial Partnership boost Growth Momentum (Safety)
                    state.stats.safety = Math.min(100, state.stats.safety + 0.01);
                    // Sustained coverage boosts Hiring Velocity (Health)
                    if (state.stats.safety > 80) {
                        state.stats.health = Math.min(100, state.stats.health + 0.01);
                    }
                } else if (tile.building.type === 'road' || tile.building.type === 'bridge') {
                    // Offering Partnership boosts PMF Signal (Education)
                    state.stats.education = Math.min(100, state.stats.education + 0.005);
                }
            }
            ...
            if (tile.zone !== 'none' && tile.building.type === 'grass') {
                ...
                // Competition Degree affects spawn chance
                const competitionDegree = state.competitionDegree ?? 32;
                const competitionFactor = competitionDegree / 100 * 1.5; // Up to 1.5x spawn boost
                ...
                // Spawn probability scales with demand and competition
                const baseSpawnChance = 0.05 * (1 + competitionFactor);
                ...
                const buildingList = tile.zone === 'residential' ? __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["RESIDENTIAL_BUILDINGS"] : tile.zone === 'commercial' ? __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["COMMERCIAL_BUILDINGS"] : __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["INDUSTRIAL_BUILDINGS"];
                    ...
                    if (canSpawnMultiTileBuilding(newGrid, x, y, candidateSize.width, candidateSize.height, tile.zone, size)) {
                        ...
                        for(let dy = 0; dy < candidateSize.height && y + dy < size; dy++){
                            if (!modifiedRows.has(y + dy)) {
                                newGrid[y + dy] = state.grid[y + dy].map((t)=>({
                                        ...t,
                                        building: {
                                            ...t.building
                                        }
            ...
            const buildingStats = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BUILDING_STATS"][tile.building.type];
            tile.pollution = Math.max(0, tile.pollution * 0.95 + (buildingStats?.pollution || 0));
            // Fire simulation
            if (state.disastersEnabled && acquisitionFireActive && tile.building.onFire) {
            ...
            if (state.disastersEnabled && acquisitionFireActive && !tile.building.onFire && tile.building.type !== 'grass' && tile.building.type !== 'water' && tile.building.type !== 'road' && tile.building.type !== 'tree' && tile.building.type !== 'empty' && tile.building.type !== 'bridge' && tile.building.type !== 'rail') {
            ...
            // Random fire start
            if (state.disastersEnabled && acquisitionFireActive && !tile.building.onFire && tile.building.type !== 'grass' && tile.building.type !== 'water' && tile.building.type !== 'road' && tile.building.type !== 'tree' && tile.building.type !== 'empty' && Math.random() < 0.00003) {
                tile.building.onFire = true;
                tile.building.fireProgress = 0;
            }
        }
    ...
    // Competition-driven revenue attack logic
    // If competition is high, competitors can attack portfolio company revenue
    const competitionDegree = state.competitionDegree ?? 32;
    if (competitionDegree > 50 && Math.random() < (competitionDegree - 50) / 500) {
        const attackAmount = Math.floor(newStats.income * (competitionDegree / 1000));
        newStats.income = Math.max(0, newStats.income - attackAmount);
        // Add notification for revenue attack
        if (Math.random() < 0.05) {
            newNotifications.push({
                id: `competition-attack-${Date.now()}`,
                title: 'Competitor Aggression',
                description: 'A rival fund is targeting your portfolio\'s market share. Revenue impacted.',
                icon: 'warning',
                timestamp: Date.now()
            });
        }
    }
    // Low competition penalty: Portfolio runway reduces due to lack of competitive pressure
    if (competitionDegree < 25 && Math.random() < (25 - competitionDegree) / 500) {
        newStats.expenses += Math.floor(newStats.expenses * 0.02);
        if (Math.random() < 0.05) {
            newNotifications.push({
                id: `competition-stagnation-${Date.now()}`,
                title: 'Market Stagnation',
                description: 'Low competition is leading to operational bloat. Portfolio burn increasing.',
                icon: 'warning',
                timestamp: Date.now()
            });
        }
    }
    // VC Fund Management Fee & Runway Dynamics
    // Management fee: 2% of AUM annually, charged monthly (0.16% per month)
    const aum = state.investorProfile?.aum_usd ?? 500_000_000;
    const monthlyFee = Math.floor(aum * 0.02 / 12 / 30); // Approximate daily fee
    newStats.expenses += monthlyFee;
    // LP Fundraising Logic
    // If competition is high and you are winning (high happiness/confidence), LPs might invest
    if (competitionDegree > 60 && newStats.happiness > 70 && Math.random() < 0.001) {
        const lpInvestment = Math.floor(aum * 0.05);
        newStats.money += lpInvestment;
        newNotifications.push({
            id: `lp-investment-${Date.now()}`,
            title: 'New LP Commitment',
            description: `Your competitive edge convinced a Limited Partner to commit $${lpInvestment.toLocaleString()} in fresh capital.`,
            icon: 'trophy',
            timestamp: Date.now()
        });
    }
...
function getConstructionSpeed(buildingType) {
...
function canPlaceMultiTileBuilding(grid, x, y, width, height, gridSize) {
    // Check bounds
    if (x + width > gridSize || y + height > gridSize) {
        return false;
    }
...
function isMergeableZoneTile(tile, zone, excludeTile, allowBuildingConsolidation) {
...
function footprintAvailable(grid, originX, originY, width, height, zone, gridSize, excludeTile, allowBuildingConsolidation) {
...
function scoreFootprint(grid, originX, originY, width, height, gridSize) {
...
function findFootprintIncludingTile(grid, x, y, width, height, zone, gridSize, allowBuildingConsolidation) {
    const candidates = [];
    ...
    return {
        originX: candidates[0].originX,
        originY: candidates[0].originY
    };
}
function applyBuildingFootprint(grid, originX, originY, buildingType, zone, level, services) {
    const size = getBuildingSize(buildingType);
    const stats = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BUILDING_STATS"][buildingType] || {
        maxPop: 0,
        maxJobs: 0,
        pollution: 0,
        landValue: 0
...
function placeBuilding(state, x, y, buildingType, zone) {
    const tile = state.grid[y]?.[x];
...
function findBuildingOrigin(grid, x, y, gridSize) {
    const tile = grid[y]?.[x];
    if (!tile) return null;
    // If this tile has an actual building (not empty), check if it's multi-tile
    if (tile.building.type !== 'empty' && tile.building.type !== 'grass' && tile.building.type !== 'water' && tile.building.type !== 'road' && tile.building.type !== 'bridge' && tile.building.type !== 'rail' && tile.building.type !== 'tree') {
        ...
        for(let dy = 0; dy < maxSize; dy++){
            for(let dx = 0; dx < maxSize; dx++){
                ...
                if (checkX >= 0 && checkY >= 0 && checkX < gridSize && checkY < gridSize) {
                    ...
                    if (checkTile.building.type !== 'empty' && checkTile.building.type !== 'grass' && checkTile.building.type !== 'water' && checkTile.building.type !== 'road' && checkTile.building.type !== 'bridge' && checkTile.building.type !== 'rail' && checkTile.building.type !== 'tree') {
                        const size = getBuildingSize(checkTile.building.type);
                        // Check if this building's footprint includes our original tile
                        if (x >= checkX && x < checkX + size.width && y >= checkY && y < checkY + size.height) {
                            return {
                                originX: checkX,
                                originY: checkY,
                                buildingType: checkTile.building.type
                            };
                        }
                    }
                }
 ...
 */ function findConnectedBridgeTiles(grid, gridSize, x, y) {
    const tile = grid[y]?.[x];
 ...
 */ function findAdjacentBridgeTiles(grid, gridSize, x, y) {
    const directions = [
        {
            dx: -1,
            dy: 0
...
function bulldozeTile(state, x, y) {
    const tile = state.grid[y]?.[x];
    if (!tile) return state;
    if (tile.building.type === 'water') return state;
    const newGrid = state.grid.map((row)=>row.map((t)=>({
...
function placeSubway(state, x, y) {
    const tile = state.grid[y]?.[x];
    if (!tile) return state;
...
function removeSubway(state, x, y) {
    const tile = state.grid[y]?.[x];
    if (!tile) return state;
    // No subway to remove
    if (!tile.hasSubway) return state;
...
function placeWaterTerraform(state, x, y) {
    const tile = state.grid[y]?.[x];
    if (!tile) return state;
...
function placeLandTerraform(state, x, y) {
    const tile = state.grid[y]?.[x];
    if (!tile) return state;
...
function generateRandomAdvancedCity(size = DEFAULT_GRID_SIZE, cityName = 'Metropolis') {
    ...
    function createAdvancedBuilding(type) {
        return {
            type,
    ...
    const placeZonedBuilding = (x, y, zone, buildingType)=>{
        const tile = grid[y]?.[x];
    ...
    const placeMultiTileBuilding = (x, y, type, zone = 'none')=>{
        ...
        for(let dy = 0; dy < buildingSize.height; dy++){
            for(let dx = 0; dx < buildingSize.width; dx++){
                ...
                if (dx === 0 && dy === 0) {
                    ...
                    const stats = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$games$2f$isocity$2f$types$2f$buildings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BUILDING_STATS"][type];
                    if (stats) {
...
function getDevelopmentBlockers(state, x, y) {
    const blockers = [];
    const tile = state.grid[y]?.[x];
    if (!tile) {
        blockers.push({
            reason: 'Invalid tile',
            details: `Tile at (${x}, ${y}) does not exist`
        });
        return blockers;
    }
    ...
    if (tile.zone === 'none') {
        blockers.push({
            reason: 'Not zoned',
            details: 'Tile has no zone assigned'
        });
        return blockers;
    }
    ...
    if (blockers.length === 0 && roadAccess && (hasUtilities || wouldBeStarter)) {
        blockers.push({
            ...
            details: wouldBeStarter && !hasUtilities ? 'Starter building can develop here without utilities! (5% chance per tick)' : 'All conditions met! Building will spawn soon (5% chance per tick)'
        });
    }
    return blockers;
}
function expandGrid(currentGrid, currentSize, expansion = 15) {
    const newSize = currentSize + expansion * 2;
    const grid = [];
    // Helper to check if position is water in the old grid
    const isOldWater = (oldX, oldY)=>{
    ...
    const getDistanceFromOriginalBoundary = (newX, newY)=>{
        const oldX = newX - expansion;
        const oldY = newY - expansion;
    ...
    const getEdgeWaterInfo = (newX, newY, sampleRadius = 10)=>{
        const oldX = newX - expansion;
        const oldY = newY - expansion;
        let waterCount = 0;
        let totalCount = 0;
    ...
    const countWaterNeighbors = (cx, cy, radius = 1)=>{
        let count = 0;
    ...
    const lakeNoise = (lx, ly)=>perlinNoise(lx, ly, expansionSeed + 3000, 3);
    ...
    const smallLakeCenters = [];
    const pondNoise = (px, py)=>perlinNoise(px, py, expansionSeed + 4000, 2);
    for(let y = minDistFromEdge; y < newSize - minDistFromEdge; y++){
        for(let x = minDistFromEdge; x < newSize - minDistFromEdge; x++){
            if (!isExpansionTile(x, y)) continue;
            if (grid[y][x].building.type === 'water') continue;
    ...
    const allLakeCenters = [
        ...selectedBigLakeCenters.map((c)=>({
                ...c,
                isBig: true
            })),
        ...selectedSmallLakeCenters.map((c)=>({
                ...c,
                isBig: false
            }))
    ];
    ...
    for (const center of allLakeCenters){
        ...
        const targetSize = center.isBig ? 25 + Math.floor(Math.random() * 46) : 4 + Math.floor(Math.random() * 12);
        const lakeTiles = [
            {
                x: center.x,
                y: center.y
        ...
        for (const [dx, dy] of directions){
            const nx = center.x + dx;
            const ny = center.y + dy;
            if (nx >= 1 && nx < newSize - 1 && ny >= 1 && ny < newSize - 1 && grid[ny][nx].building.type !== 'water') {
                const dist = Math.sqrt(dx * dx + dy * dy);
                const noise = lakeNoise(nx, ny);
                candidates.push({
                    x: nx,
        ...
        while(lakeTiles.length < targetSize && candidates.length > 0){
            ...
            for (const [dx, dy] of directions){
                const nx = picked.x + dx;
                const ny = picked.y + dy;
                if (nx >= 1 && nx < newSize - 1 && ny >= 1 && ny < newSize - 1 && grid[ny][nx].building.type !== 'water' && !lakeTiles.some((t)=>t.x === nx && t.y === ny) && !candidates.some((c)=>c.x === nx && c.y === ny)) {
                    const dist = Math.sqrt((nx - center.x) ** 2 + (ny - center.y) ** 2);
                    const noise = lakeNoise(nx, ny);
                    candidates.push({
                        x: nx,
            ...
            for(let attempts = 0; attempts < 50 && !foundStart; attempts++){
                ...
                if (isExpansionTile(testX, testY) && grid[testY][testX].building.type === 'water') {
                    ...
                    for (const [dx, dy] of [
                        ...
                        const nx = testX + dx;
                        const ny = testY + dy;
                        if (nx >= 0 && ny >= 0 && nx < newSize && ny < newSize && grid[ny][nx].building.type !== 'water') {
...
function shrinkGrid(currentGrid, currentSize, shrinkAmount = 15) {
...
"[project]/src/lib/renderConfig.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const SPRITE_PACK_SPRITES4 = {
    ...
    parksVerticalOffsets: {
        ...
        playground_small: -0.25,
        playground_large: -0.60,
        baseball_field_small: -0.55,
        soccer_field_small: -0.20,
        football_field: -0.55,
        baseball_stadium: -1.35,
        community_center: -0.2,
        office_building_small: -0.3,
        swimming_pool: -0.20,
        skate_park: -0.25,
        mini_golf_course: -0.60,
        bleachers_field: -0.2,
        go_kart_track: -0.30,
        amphitheater: -0.40,
        greenhouse_garden: -0.75,
        animal_pens_farm: -0.25,
        cabin_house: -0.2,
        campground: -0.15,
        marina_docks_small: -0.45,
        pier_large: -0.1,
        roller_coaster_small: -0.50,
        community_garden: -0.15,
        pond_park: -0.15,
        park_gate: -0.15,
        mountain_lodge: -0.55,
        mountain_trailhead: -1.0
    },
    parksHorizontalOffsets: {
    },
    parksScales: {
        baseball_stadium: 0.81,
        baseball_field_small: 0.855,
        basketball_courts: 0.9,
        football_field: 0.855,
        swimming_pool: 0.90,
        soccer_field_small: 0.95,
        go_kart_track: 0.92,
        mini_golf_course: 0.95,
        amphitheater: 0.90,
        greenhouse_garden: 0.90
    },
...
const SPRITE_PACK_SPRITES4_HARRY = {
...
const SPRITE_PACK_SPRITES4_CHINA = {
    ...SPRITE_PACK_SPRITES4,
    id: 'sprites4-china',
    name: 'Chinese Theme',
...
const DEFAULT_SPRITE_PACK_ID = 'sprites4';
function getSpritePack(id) {
...
let _activeSpritePack = SPRITE_PACKS[0];
function setActiveSpritePack(pack) {
    _activeSpritePack = pack;
}
function getActiveSpritePack() {
...
const SPRITE_ORDER = _activeSpritePack.spriteOrder;
const SPRITE_VERTICAL_OFFSETS = new Proxy({}, {
    get (_, key) {
...
const SPRITE_HORIZONTAL_OFFSETS = new Proxy({}, {
    get (_, key) {
...
const BUILDING_TO_SPRITE = new Proxy({}, {
    get (_, key) {
...
function getSpriteCoords(buildingType, spriteSheetWidth, spriteSheetHeight, pack) {
...
function getSpriteOffsets(buildingType, pack) {
    const activePack = pack || _activeSpritePack;
    const spriteKey = activePack.buildingToSprite[buildingType];
    return {
...
"[project]/src/lib/multiplayer/types.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
"[project]/src/lib/multiplayer/database.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const supabaseKey = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
// Lazy init: only create client when Supabase is configured
const supabase = supabaseUrl && supabaseKey ? (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$supabase$2f$supabase$2d$js$2f$dist$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$locals$3e$__["createClient"])(supabaseUrl, supabaseKey) : null;
// Maximum city size limit for Supabase storage (20MB)
const MAX_CITY_SIZE_BYTES = 20 * 1024 * 1024; // 20MB
class CitySizeLimitError extends Error {
    sizeBytes;
    limitBytes;
    constructor(sizeBytes, limitBytes = MAX_CITY_SIZE_BYTES){
 ...
 */ function checkCitySize(compressed) {
...
async function createGameRoom(roomCode, cityName, gameState) {
    if (!supabase) return false;
    try {
        // PERF: Both JSON.stringify and lz-string compression happen in the worker
        const compressed = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$saveWorkerManager$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["serializeAndCompressForDBAsync"])(gameState);
...
async function loadGameRoom(roomCode) {
    ...
    try {
        const { data, error } = await supabase.from('game_rooms').select('game_state, city_name').eq('room_code', roomCode.toUpperCase()).single();
        if (error || !data) {
            console.error('[Database] Failed to load room:', error);
            return null;
        }
        const decompressed = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lz$2d$string$2f$libs$2f$lz$2d$string$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["decompressFromEncodedURIComponent"])(data.game_state);
        if (!decompressed) {
...
async function updateGameRoom(roomCode, gameState) {
    if (!supabase) return false;
    try {
        // PERF: Both JSON.stringify and lz-string compression happen in the worker
        const compressed = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$saveWorkerManager$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["serializeAndCompressForDBAsync"])(gameState);
...
async function roomExists(roomCode) {
    if (!supabase) return false;
    try {
        const { data, error } = await supabase.from('game_rooms').select('room_code').eq('room_code', roomCode.toUpperCase()).single();
        return !error && !!data;
    } catch  {
        return false;
    }
}
async function updatePlayerCount(roomCode, count) {
    if (!supabase) return;
    try {
...
"[project]/src/lib/multiplayer/supabaseProvider.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$supabase$2f$supabase$2d$js$2f$dist$2f$index$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$locals$3e$__ = __turbopack_context__.i("[project]/node_modules/@supabase/supabase-js/dist/index.mjs [app-client] (ecmascript) <locals>");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/multiplayer/types.ts [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$database$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/multiplayer/database.ts [app-client] (ecmascript)");
...
class MultiplayerProvider {
    ...
    constructor(options){
        ...
        this.peerId = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generatePlayerId"])();
        this.gameState = options.initialGameState || null;
        this.isCreator = !!options.initialGameState;
        // Create player info
        this.player = {
            id: this.peerId,
            name: options.playerName || (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generatePlayerName"])(),
            color: (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$types$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["generatePlayerColor"])(),
            joinedAt: Date.now(),
            isHost: false
        };
        // Add self to players
    ...
    async connect() {
        ...
        if (this.isCreator && this.gameState) {
            ...
            try {
                const success = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$database$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["createGameRoom"])(this.roomCode, this.options.cityName, this.gameState);
                if (!success) {
                    this.options.onError?.((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$gt$2d$next$2f$dist$2f$index$2e$client$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["msg"])('Failed to create room in database'));
                    throw new Error((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$gt$2d$next$2f$dist$2f$index$2e$client$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["msg"])('Failed to create room in database'));
                }
            } catch (e) {
                if (e instanceof __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$database$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["CitySizeLimitError"]) {
            ...
            const roomData = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$database$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["loadGameRoom"])(this.roomCode);
            if (!roomData) {
                this.options.onError?.((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$gt$2d$next$2f$dist$2f$index$2e$client$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["msg"])('Room not found'));
                throw new Error((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$gt$2d$next$2f$dist$2f$index$2e$client$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["msg"])('Room not found'));
            }
        ...
        this.channel// Presence: track who's in the room
        .on('presence', {
            event: 'sync'
        }, ()=>{
            ...
            (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$database$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["updatePlayerCount"])(this.roomCode, this.players.size);
        }).on('presence', {
            event: 'join'
        }, ({ key, newPresences })=>{
   ...
   */ updateGameState(state) {
    ...
    saveStateToDatabase(state) {
        ...
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$database$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["updateGameRoom"])(this.roomCode, state).catch((e)=>{
            if (e instanceof __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$multiplayer$2f$database$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["CitySizeLimitError"]) {
...
async function createMultiplayerProvider(options) {
...
"[project]/src/lib/vcSimulation.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = /*#__PURE__*/ __turbopack_context__.i("[project]/node_modules/next/dist/build/polyfills/process.js [app-client] (ecmascript)");
const API_BASE_URL = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].env.NEXT_PUBLIC_VC_SIM_API_BASE_URL?.replace(/\/$/, '') || 'http://localhost:8000';
const INVESTOR_DOMAIN_STORAGE_KEY = 'isobusiness-investor-domain';
const SIM_SESSION_STORAGE_KEY = 'isobusiness-sim-session-id';
function buildFallbackLogo(name) {
    const initials = name.split(' ').filter(Boolean).slice(0, 2).map((part)=>part[0]?.toUpperCase() ?? '').join('') || 'VC';
...
const FALLBACK_SIMULATION_STATE = {
...
function normalizeCompany(value, index) {
    ...
    const logoUrl = typeof record.logo_url === 'string' && record.logo_url.length > 0 ? record.logo_url : buildFallbackLogo(name);
    return {
...
function normalizeReplayTick(value, index) {
    const record = asRecord(value);
    if (!record) {
        return null;
    }
    return {
        ...
        companies: Array.isArray(record.companies) ? record.companies.map(normalizeCompany).filter((company)=>company !== null) : [],
        interactions: Array.isArray(record.interactions) ? record.interactions.map(normalizeInteraction).filter((interaction)=>interaction !== null) : []
    };
}
function normalizeSimulationState(payload) {
    ...
    const companies = Array.isArray(snapshot?.companies) ? snapshot.companies.map(normalizeCompany).filter((company)=>company !== null) : [];
    const interactions = Array.isArray(snapshot?.interactions) ? snapshot.interactions.map(normalizeInteraction).filter((interaction)=>interaction !== null) : [];
    return {
        ...
        deterministic_mode: typeof record.deterministic_mode === 'boolean' ? record.deterministic_mode : FALLBACK_SIMULATION_STATE.deterministic_mode,
        current_tick: asNumber(record.current_tick, FALLBACK_SIMULATION_STATE.current_tick),
        restored_from_db: typeof record.restored_from_db === 'boolean' ? record.restored_from_db : FALLBACK_SIMULATION_STATE.restored_from_db,
        ...
        current_snapshot: {
            ...
            interactions,
            investors: Array.isArray(snapshot?.investors) ? snapshot.investors.map((investor)=>asRecord(investor) ?? {}) : [],
            metrics: normalizeMetrics(snapshot?.metrics),
            updated_at: asString(snapshot?.updated_at, fallbackSnapshot.updated_at)
        },
        recent_replay: Array.isArray(record.recent_replay) ? record.recent_replay.map(normalizeReplayTick).filter((tick)=>tick !== null) : []
    };
}
async function fetchSimulationState(signal) {
    const query = new URLSearchParams();
    query.set('advance', 'true');
    if ("TURBOPACK compile-time truthy", 1) {
        const investorDomain = localStorage.getItem(INVESTOR_DOMAIN_STORAGE_KEY);
        const existingSessionId = localStorage.getItem(SIM_SESSION_STORAGE_KEY);
        if (investorDomain) query.set('investor_domain', investorDomain);
        if (existingSessionId) query.set('session_id', existingSessionId);
    }
    const response = await fetch(`${API_BASE_URL}/api/v1/simulation/state?${query.toString()}`, {
        cache: 'no-store',
        signal
    });
    if (!response.ok) {
    ...
    const normalized = normalizeSimulationState(await response.json());
    if ("TURBOPACK compile-time truthy", 1) {
        localStorage.setItem(SIM_SESSION_STORAGE_KEY, normalized.session_id);
    }
    return normalized;
}
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
...
"[project]/src/lib/investorProfiles.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "INVESTOR_DOMAIN_STORAGE_KEY",
    ()=>INVESTOR_DOMAIN_STORAGE_KEY,
    "INVESTOR_PORTFOLIO_COMPETITOR_JOIN_SQL",
    ()=>INVESTOR_PORTFOLIO_COMPETITOR_JOIN_SQL,
    "resolveInvestorProfileByDomain",
    ()=>resolveInvestorProfileByDomain
]);
const INVESTOR_DOMAIN_STORAGE_KEY = 'isobusiness-selected-investor-domain';
const Y_COMBINATOR_PROFILE = {
    id: 'y-combinator',
    name: 'Y Combinator',
    domain: 'ycombinator.com',
    dryPowderUsd: 2_500_000_000,
    portfolio: [
        {
            id: 'airbnb',
            name: 'Airbnb',
...
const INVESTOR_PROFILES = [
    Y_COMBINATOR_PROFILE
];
function normalizeDomain(value) {
    return value.trim().toLowerCase().replace(/^https?:\/\//, '').replace(/^www\./, '').replace(/\/+$/, '');
}
function resolveInvestorProfileByDomain(domain) {
    if (!domain) return null;
    const normalized = normalizeDomain(domain);
    if (!normalized) return null;
    return INVESTOR_PROFILES.find((profile)=>normalizeDomain(profile.domain) === normalized) ?? null;
}
const INVESTOR_PORTFOLIO_COMPETITOR_JOIN_SQL = `
SELECT
  i.id AS investor_id,
  i.name AS investor_name,
  i.domain AS investor_domain,
  p.id AS portfolio_company_id,
  p.name AS portfolio_company_name,
  p.domain AS portfolio_company_domain,
  p.sector AS portfolio_sector,
  c.id AS competitor_id,
  c.name AS competitor_name,
  c.domain AS competitor_domain
FROM investors i
LEFT JOIN investor_portfolio_companies p
  ON p.investor_id = i.id
LEFT JOIN investor_competitors c
  ON c.investor_id = i.id
WHERE i.domain = $1;
`.trim();
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
...
//# sourceMappingURL=src_lib_c8e60e94._.js.map